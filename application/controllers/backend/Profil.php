...
class Profil extends CI_Controller{
    ...
    function change() {
    	if($this->session->userdata('access')=='1') 
		{
				...
				$id = $this->input->post('id');
				$idvendor = $this->profil_setting_model->get_detail_profil($id)->row()->vendor;
				...
				$user_email = $this->input->post('user_email');
							...
							$conf_pass = $this->input->post('conf_pass');
								if(empty($pass) && empty($conf_pass)){
									...
									$vendordata['nama'] = $this->input->post('user_name');
									$vendordata['npwp'] = $this->input->post('user_email');
									$updatevendor =  $this->vendors_model->update_entry($idvendor, $vendordata);

									if ($this->profil_setting_model->update_entry($id, $ajax_data) && $updatevendor) {
										$data = $this->profil_setting_model->get_detail_profil($id)->result();
										echo json_encode($data);
									} else {
										$data = $this->profil_setting_model->get_detail_profil($id)->result();
